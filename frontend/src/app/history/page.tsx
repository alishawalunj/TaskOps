...
// import { usePreviousTasks } from '../hooks/useTasksQueries';
...
//   const { previousTasks, loading, error, refetch } = usePreviousTasks(userId || "");
...
//           <h1 className="text-5xl font-bold text-green-400 py-10">Your history</h1>
//         </div>
...
export default function History() {
  ...
  // NEW TASK STATE (kept because you're using it elsewhere / future use)
   ...
   * Hook will NOT refetch endlessly because we don't mirror state
   */
  const {
    previousTasks,
    loading,
    error,
  } = usePreviousTasks(userId ?? "");

  /**
   * Guard: userId not loaded yet
   */
  if (!userId) {
    return <div className="text-green-400 p-10">Loading user...</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col relative overflow-x-hidden">
      ...
      >
        {!isSidebarOpen && <CiMenuBurger className="w-12 h-6" />}
      </button>
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-6">
          {previousTasks && previousTasks.length > 0 ? (
            previousTasks.map((task: TaskResponseDTO) => (
              <TaskCircle
                key={task.taskId}
                taskId={task.taskId}
                name={task.name}
